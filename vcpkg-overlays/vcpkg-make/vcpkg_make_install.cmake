...
function(vcpkg_make_install)
    ...
    # --- PATCH: fallback pour le cas où --prefix n'a pas été transmis à configure
    # (bug vcpkg-make@2026-01-01 : les options par défaut, dont --prefix, ne sont
    # pas propagées, donc `make install` retombe sur le /usr/local d'autotools)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/usr/local")
        file(GLOB _children RELATIVE "${CURRENT_PACKAGES_DIR}/usr/local" "${CURRENT_PACKAGES_DIR}/usr/local/*")
        foreach(_child ${_children})
            file(COPY "${CURRENT_PACKAGES_DIR}/usr/local/${_child}" DESTINATION "${CURRENT_PACKAGES_DIR}")
        endforeach()
